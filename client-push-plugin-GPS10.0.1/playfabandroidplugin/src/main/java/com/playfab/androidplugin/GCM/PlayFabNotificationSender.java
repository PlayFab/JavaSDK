...
public class PlayFabNotificationSender {
    public static void sendNotification(Context intent, PlayFabNotificationPackage pushNotificationPackage) {
        try {
            PendingIntent pendingIntent = PendingIntent.getActivity(intent, 8273,
            ...
            String appIcon = pushNotificationPackage.Icon == null || pushNotificationPackage.Icon.isEmpty() ? sharedPreferences.getString(PlayFabAndroidPlugin.PROPERTY_APP_ICON, "app_icon") : pushNotificationPackage.Icon;
            ...
            String title = pushNotificationPackage.Title == null || pushNotificationPackage.Title.isEmpty() ? sharedPreferences.getString(PlayFabAndroidPlugin.PROPERTY_GAME_TITLE, "") : pushNotificationPackage.Title;
    ...
    public static PlayFabNotificationPackage createNotificationPackage(Context ctx, String message, int id, Date schedule, boolean isLocal) {
        PlayFabNotificationPackage pac = createNotificationPackage(ctx,message,0);
        SimpleDateFormat sdf = new SimpleDateFormat(PlayFabConst.DATE_LOCAL_FORMAT);
        if(!isLocal){
            sdf.setTimeZone(TimeZone.getTimeZone("UTC"));
        }
        pac.setScheduleDate(sdf.format(schedule) + (isLocal ? "" : "Z"));
        return pac;
    }

    public static PlayFabNotificationPackage createNotificationPackageFromMessage(Context ctx, String message, int id, Date schedule, boolean isLocal){
        PlayFabNotificationPackage pac = createNotificationPackageFromMessage(ctx,message,0);
        SimpleDateFormat sdf = new SimpleDateFormat(PlayFabConst.DATE_LOCAL_FORMAT);
        if(!isLocal){
            sdf.setTimeZone(TimeZone.getTimeZone("UTC"));
        }
    ...
    public static PlayFabNotificationPackage createNotificationPackageFromMessage(Context ctx, String message, int id){
        PlayFabNotificationPackage output = new PlayFabNotificationPackage();
        output.setMessage(message, id);
        return output;
    }
    ...
    public static void send(Context intent, PlayFabNotificationPackage notifyPackage) {
        try {
            ...
            Log.d(PlayFabAndroidPlugin.TAG,"Manager will delay delivery for "+delayMillis+"ms");
