...
public class PlayFabApiTest
{
    ...
    public static void oneTimeSetUp() {
        ...
        String devSecretKey = env.get("DEVELOPER_SECRET_KEY_JAVA_SDK");
        if (devSecretKey != null && !devSecretKey.equals("")) {
            resultData.developerSecretKey = devSecretKey;
        }
    ...
    public void GetEntityToken()
    {
        ...
        PlayFabAuthenticationModels.GetEntityTokenRequest request = new PlayFabAuthenticationModels.GetEntityTokenRequest();
    ...
    public void ObjectApi()
    {
        ...
        if (getObjResult.Result.Objects.size() == 1 && getObjResult.Result.Objects.containsKey(TEST_DATA_KEY))
            tempObjResult = getObjResult.Result.Objects.get(TEST_DATA_KEY);
        if (tempObjResult != null)
            testCounterValueExpected = Integer.parseInt(tempObjResult.EscapedDataObject);
        testCounterValueExpected = (testCounterValueExpected + 1) % 100; // This test is about the expected value changing - but not testing more complicated issues like bounds
        ...
        PlayFabResult<PlayFabDataModels.SetObjectsResponse> setObjResult = PlayFabDataAPI.SetObjects(setObjRequest);
        VerifyResult(setObjResult, true);

        getObjResult = PlayFabDataAPI.GetObjects(getRequest);
        VerifyResult(getObjResult, true);
        ...
        if (getObjResult.Result.Objects.size() == 1 && getObjResult.Result.Objects.containsKey(TEST_DATA_KEY))
            tempObjResult = getObjResult.Result.Objects.get(TEST_DATA_KEY);
        if (tempObjResult != null)
            testCounterValueActual = Integer.parseInt(tempObjResult.EscapedDataObject);
        assertEquals(testCounterValueExpected, testCounterValueActual);
    }
}
